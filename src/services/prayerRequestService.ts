 ...
 * Handles manual prayer attendance requests
 * MIGRATED: Now uses local API endpoints
 */

import type { MissedPrayerRequest } from '@/types';

interface DbMissedPrayerRequest {
    id: string;
    mentee_id: string;
    mentee_name: string;
    mentor_id: string;
...
export const getAllMissedPrayerRequests = async (): Promise<MissedPrayerRequest[]> => {
    try {
        ...
        return data.map((req: DbMissedPrayerRequest) => ({
            id: req.id,
...
export const createMissedPrayerRequest = async (
    ...
    if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Gagal membuat pengajuan sholat terlewat');
    }
...
export const updateMissedPrayerRequest = async (
    ...
    const dbUpdates: Partial<DbMissedPrayerRequest> & { id: string } = { id: requestId };
