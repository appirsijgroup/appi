...
import { convertToCamelCase, type RawSqlEmployee } from './employeeService';
...
export async function getPaginatedEmployees(
  ...
  const rawEmployees = (data.employees || []) as RawSqlEmployee[];

  return {
    ...data,
    employees: rawEmployees.map((emp) => convertToCamelCase(emp))
  };
}
 ...
 */
export async function getEmployeeDetail(id: string): Promise<Employee> {
  ...
  return convertToCamelCase(result.data || result.employees?.[0] || result);
}
