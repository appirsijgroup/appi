...
export const DataLoader: React.FC<DataLoaderProps> = ({ children }) => {
    ...
    const { initializeFromEmployee, load: loadMutabaahSettings } = useMutabaahStore();
        ...
        const loadEssentialData = async () => {
            try {
                ...
                const loadPromises = [
                    ...
                    loadAnnouncements().catch((err: unknown) => {
                        // Don't throw - allow other data to load
                    }),

                    // Hospitals - needed for various features
                    loadHospitals().catch((err: unknown) => {
                        // Don't throw - allow other data to load
                    }),

                    // Mutabaah Settings
                    loadMutabaahSettings().catch((err: unknown) => { }),
                ];
