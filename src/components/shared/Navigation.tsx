...
const Navigation: React.FC<NavigationProps> = ({ navItems, isOpen, setIsOpen, unreadAnnouncementsCount, onLogout }) => {
  ...
  const handleItemClick = useCallback((href: string, e?: React.MouseEvent) => {
    ...
    // 🔥 FIX: We NO LONGER call setIsOpen(false) here immediately.
    // Calling it here can cause the element to unmount/hide before the click 
    // event propagates to the Next.js Link handler, causing navigation to fail.
    // Instead, we let the useEffect below handle closing when pathname changes.

    // Clear click state after a short delay
    setTimeout(() => {
      setClickedItem(null);
    }, 400);
  }, []);

  // 🔥 NEW: Automatically close sidebar on mobile when navigation successfully happens
  React.useEffect(() => {
    if (isOpen) {
      setIsOpen(false);
    }
  ...
  const navItemClass = (href: string) => {
    ...
    return `w-full flex items-center justify-start text-left gap-4 px-4 py-2 text-sm font-bold rounded-xl cursor-pointer
      transition-colors duration-200 ease-out relative overflow-hidden
      ${active
        ? 'bg-linear-to-r from-teal-500 to-emerald-600 text-white shadow-lg shadow-teal-500/20'
        : 'text-slate-300 hover:bg-white/5 hover:text-white'
      }
      ${isClicked ? 'bg-white/10 brightness-125' : ''}
    `;
  };
          ...
          <div className="shrink-0 border-t border-white/10 p-4">
            <button
              ...
              className={`w-full flex items-center justify-start gap-4 px-4 py-3 text-sm font-semibold rounded-lg
                ...
                ${clickedItem === 'logout' ? 'brightness-110' : ''}
