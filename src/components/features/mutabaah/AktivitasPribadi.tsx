...
export const RiwayatPengajuan: React.FC<{
    ...
    const combinedHistory = useMemo(() => {
        ...
        const tadarus = (tadarusRequests || [])
            .filter(r => r.menteeId === employeeId)
            .map(r => ({
                id: r.id,
        ...
        // 2. Map Missed Prayer Requests
        const missedPrayers = (missedPrayerRequests || [])
            .filter(r => r.menteeId === employeeId)
            .map(r => ({
                id: r.id,
        ...
        // 3. Map Monthly Report Submissions
        const reports = (monthlyReportSubmissions || [])
            .filter(s => s.menteeId === employeeId)
            .map(s => ({
                id: s.id,
                type: 'Laporan Bulanan' as const,
                date: s.monthKey + '-01',
                periode: `Periode: ${new Date(s.monthKey + '-02').toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}`,
        ...
        const all = [...tadarus, ...missedPrayers, ...reports];
    ...
    const filteredHistory = useMemo(() => {
        return combinedHistory.filter(item => {
            ...
            // Specifically for monthly reports, we check monthKey instead
            if (item.type === 'Laporan Bulanan') {
                const reportDate = new Date(item.originalData.monthKey + '-02');
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'approved':
                return <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-green-500/20 text-green-300">DISETUJUI</span>;
            case 'pending':
            case 'pending_mentor':
            case 'pending_supervisor':
            case 'pending_kaunit':
                return <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-yellow-500/20 text-yellow-300">MENUNGGU</span>;
            case 'rejected':
            default:
                if (status && status.startsWith('rejected_')) {
                    return <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-red-500/20 text-red-300">DITOLAK</span>;
                }
                if (status === 'rejected') return <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-red-500/20 text-red-300">DITOLAK</span>;
                return <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-gray-500/20 text-gray-300">{status?.toUpperCase() || 'N/A'}</span>;
        }
    };

    return (
        <div className="bg-gray-800/50 p-6 rounded-2xl shadow-lg border border-white/10 animate-view-change">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h3 className="text-xl font-bold text-white">Status Pengajuan</h3>
                </div>
                <div className="flex items-center justify-between bg-black/20 p-1 rounded-full border border-white/5 w-full sm:w-auto">
                    <button onClick={() => navigateMonth('prev')} className="px-3 py-1.5 rounded-full hover:bg-white/10 transition-colors">
                        &larr;
                    </button>
                    <span className="font-semibold text-sm text-teal-300 px-4 w-40 text-center">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full text-sm text-left text-white">
                    <thead className="bg-white/10 text-xs uppercase text-blue-200">
                        <tr>
                            <th scope="col" className="px-4 py-3 whitespace-nowrap">Tanggal Pengajuan</th>
                            <th scope="col" className="px-4 py-3 whitespace-nowrap">Jenis</th>
                            ...
                            <th scope="col" className="px-4 py-3 whitespace-nowrap">Catatan/Alasan</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filteredHistory.length > 0 ? filteredHistory.map(item => (
                            <tr key={item.id} className="border-b border-gray-700 hover:bg-white/5 transition-colors">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold tracking-tight 
                                        ${item.type === 'Tadarus/BBQ' ? 'bg-teal-500/20 text-teal-300' :
                                            item.type === 'Presensi Terlewat' ? 'bg-purple-500/20 text-purple-300' :
                                                'bg-blue-500/20 text-blue-300'}`}>
                                        {item.type}
                                    </span>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap font-medium">{item.periode}</td>
                            ...
                            <tr><td colSpan={5} className="text-center p-12 text-blue-200/50 italic">Tidak ada pengajuan pada periode ini.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
...
export interface AktivitasPribadiViewProps {
    ...
    tadarusRequests?: TadarusRequest[]; // Optional for backward compatibility
    missedPrayerRequests?: MissedPrayerRequest[]; // Optional
}
