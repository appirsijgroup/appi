...
const MonthlyActivities: React.FC<MonthlyActivitiesProps> = ({
    ...
    const currentMonthlySubmission = useMemo(() => {
        ...
        return (monthlyReportSubmissions || []).find(s => s.monthKey.trim() === target);
    ...
    const handleProgressChange = (day: string, activityId: string, value: boolean) => {
        ...
        // 🔥 OPTIMIZATION: Use spread operators instead of JSON.parse(JSON.stringify())
        // Object.assign or spread is much faster and cleaner
        const currentDayProgress = progress[day] || {};

        const newDayProgress = { ...currentDayProgress };
        if (value) {
            newDayProgress[activityId] = true;
        } else {
            delete newDayProgress[activityId];
        }

        const newMonthProgress = {
            ...progress,
            [day]: newDayProgress
        };
    ...
    }, [dailyActivitiesConfig, weeks, currentWeekIndex, activityProgressCounts, weeklyAchievedCounts]);
    ...
    const canSubmitThisMonth = useMemo(() => {
        ...
        // Hanya bisa dikirim di akhir bulan (tanggal terakhir bulan tersebut)
                ...
                <div className="text-blue-200 mb-4">
                    {canSubmitThisMonth ? (
                        ...
                        <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                            ...
                            <p className="text-sm text-yellow-100/80">Pengiriman laporan baru dibuka pada akhir bulan (tanggal <strong>{daysInMonth}</strong>).<br /><br />Silakan lanjutkan pengisian mutaba'ah harian Anda.</p>
                        </div>
                    )}
                </div>
                                            ...
                                            <Fragment key={category}>
                                                ...
                                                {activities.map(activity => (
                                                    <tr key={activity.id} className="group border-b border-gray-700">
                                                        ...
                                                        {selectedWeekDays.map(day => {
                                                            const dayKey = day.toString().padStart(2, '0');
                                                            const isChecked = !!progress[dayKey]?.[activity.id];
                                                            return (
                                                                <td
                                                                    key={day}
                                                                    className={`text-center border-l border-gray-700 cursor-default transition-colors ${isCurrentMonthView && day === todayDay ? 'bg-teal-900/40' : ''}`}
                                                                >
                    ...
                    {activeTab === 'monthly' && (
                        <div className="animate-view-change">
                                                    ...
                                                    <tr key={activity.id} className="group border-b border-gray-700">
                                                        ...
                                                        {Array.from({ length: daysInMonth }, (_, i) => (i + 1).toString().padStart(2, '0')).map(dayKey => {
                                                            ...
                                                            return (
                                                                <td
                                                                    key={dayKey}
                                                                    className={`text-center border-l border-gray-700 cursor-default transition-colors ${isCurrentMonthView && parseInt(dayKey) === todayDay ? 'bg-teal-900/40' : ''}`}
                                                                >
                                                                    <div className="w-full h-full flex items-center justify-center py-3">
                                                                        {isChecked ? <CheckSquare className="w-6 h-6 text-teal-400" /> : <Square className="w-6 h-6 text-gray-600" />}
                                                                    </div>
                            ...
                            <div className="mt-8 flex flex-col items-center gap-4">
                                {currentMonthlySubmission ? (
                                    <div className={`px-2.5 py-1.5 text-sm font-semibold rounded-full ${statusConfig[currentMonthlySubmission.status]?.color || 'bg-blue-500/20 text-blue-300'}`}>
                                        Status Laporan: {statusConfig[currentMonthlySubmission.status]?.label || currentMonthlySubmission.status}
                                    </div>
                                ) : (
                                    <button onClick={handleSubmission} className="px-6 py-2 bg-teal-500 hover:bg-teal-400 text-white font-bold rounded-lg shadow-lg flex items-center gap-2">
                                        <Send className="w-5 h-5" /> Kirim Laporan Bulan Ini
                                    ...
                                    {[
                                        ...
                                        <div key={idx} className="flex flex-col text-center px-2 py-4 bg-black/20 rounded-lg h-48 justify-between flex-1 w-full sm:min-w-[150px] border border-white/5">
                                            ...
                                            <div className="flex-1 flex items-center justify-center my-2">
                                                {person.signature ? (
                                                    <NextImage src={person.signature} alt={person.role} width={100} height={64} className="h-16 w-auto object-contain brightness-110" unoptimized />
