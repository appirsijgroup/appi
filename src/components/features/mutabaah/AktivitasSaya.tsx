...
import { type Employee, MonthlyReportSubmission, DailyActivity, TadarusRequest, MissedPrayerRequest, TadarusSession, Hospital, Attendance, HistoryRecord } from '@/types';
import { Pencil, ShieldCheck, CheckSquare, BookOpen } from 'lucide-react';
import { AktivitasPribadiView, RiwayatBacaan, RiwayatPengajuan } from '@/components/features/mutabaah/AktivitasPribadi';
...
interface AktivitasSayaProps {
    ...
    onCreateTadarusSession: (data: TadarusSession) => void;
...
const TabButton: React.FC<{
    ...
    <button
        onClick={onClick}
        className={`grow flex flex-col sm:flex-row items-center justify-center gap-2 py-4 px-6 text-sm font-bold border-b-2 transition-all duration-200 whitespace-nowrap relative active:scale-95
          ${active
...
const AktivitasSaya: React.FC<AktivitasSayaProps> = (props) => {
    const [activeTab, setActiveTab] = useState<'aktivitas-pribadi' | 'riwayat-bacaan' | 'status-pengajuan'>(
        (props.initialTab as 'aktivitas-pribadi' | 'riwayat-bacaan' | 'status-pengajuan') || 'aktivitas-pribadi'
    );

    // Role calculations
    const hasMentorRole = props.employee.canBeMentor === true;
    const hasApprovalRole = props.employee.canBeKaUnit === true;
            ...
            case 'status-pengajuan':
                return (
                    <RiwayatPengajuan
                        ...
                        monthlyReportSubmissions={props.monthlyReportSubmissions}
                    />
                );
            default:
                return null;
            ...
            <nav className="border-b border-white/20">
                <div className="overflow-x-auto overflow-y-hidden touch-pan-x">
                    <div className="flex items-center gap-2 -mb-px min-w-max">
                        ...
                        <TabButton label="Status Pengajuan" icon={ShieldCheck} active={activeTab === 'status-pengajuan'} onClick={() => setActiveTab('status-pengajuan')} />
                    </div>
                </div>
            </nav>
            <div className="mt-6">
