...
import { PdfIcon, ChartBarIcon, DocumentTextIcon, ArrowLeftIcon, CheckIcon, XIcon, ClockIcon } from '@/components/ui/Icons';
...
const generateTranscriptPdf = async (
    ...
    if (performanceData && performanceData.categories.length > 0) {
        const tableBody: any[][] = [];
        performanceData.categories.forEach((cat) => {
            tableBody.push([
...
export const CeklisMutabaahView: React.FC<{
    employee: Employee;
    dailyActivitiesConfig: DailyActivity[];
    selectedMonth: Date;
    allUsersData: Record<string, { employee: Employee; attendance: Record<string, any>; history: Record<string, any>; }>;
...
}> = ({ employee, dailyActivitiesConfig, selectedMonth, allUsersData, onBack, hospital, isMentorApproved, reportContent }) => {
    ...
    const getName = useCallback((id: string | null | undefined) => {
        if (!id) return null;
...
const TranskripNilaiView: React.FC<TranskripNilaiViewProps> = ({ employee, allUsersData, selectedMonth, performanceData, ipForMonth, signatoryName, signatoryNip, signatoryTitle, signatorySignature, hospital, isDownloading, setIsDownloading }) => {
    ...
    const bossInfo = useMemo(() => {
        const getBossName = (id: string | null | undefined) => {
            if (!id) return 'Belum Diatur';
            const strId = String(id).trim();
                        ...
                        <div className="flex justify-between gap-12">
                            <div className="flex-1 text-center">
                                ...
                                <div className="h-16 flex items-center justify-center">
                                    {signatorySignature && (
                                        <NextImage src={signatorySignature} alt="Tanda Tangan Signatory" width={100} height={64} className="h-16 w-auto object-contain brightness-110" unoptimized />
                                    )}
                                </div>
                            ...
                            <div className="flex-1 text-center">
                                ...
                                <div className="h-16 flex items-center justify-center">
                                    {employee.signature && (
                                        <NextImage src={employee.signature} alt="Tanda Tangan" width={100} height={64} className="h-16 w-auto object-contain brightness-110" unoptimized />
                                    )}
                                </div>
...
const RapotView: React.FC<RapotViewProps> = ({ employee, dailyActivitiesConfig, allUsersData, hospitals }) => {
    ...
    const getStatusDisplay = (monthKey: string) => {
        ...
        switch (submission.status) {
            ...
            default:
                if (submission.status && submission.status.startsWith('rejected')) {
    ...
    const calculateYearlyPerformance = (year: number) => {
        ...
        // 🔥 Akumulasi data Setahun Penuh (Jan-Des), tapi Score Pro-Rata
        const maxMonthIdx = 12;
        ...
        const categoriesRaw: Record<string, {
            name: string;
            details: Record<string, { title: string; yearlyTarget: number; yearlyAchieved: number; proRataTarget: number }>;
        }> = {};

        monthsToCalculate.forEach((mKey: string) => {
            const mProgress = employee.monthlyActivities?.[mKey] || {};
            const monthlyReports = (employee as unknown as { _monthlyReportsDataCache?: Record<string, unknown> })._monthlyReportsDataCache?.[mKey] || {};
            ...
            dailyActivitiesConfig.forEach(act => {
                ...
                if (!categoriesRaw[act.category].details[act.id]) {
                    categoriesRaw[act.category].details[act.id] = {
                        title: act.title,
                        // 🔥 FIX: Target Setahun (Fixed 12 bulan) agar visual konsisten
                        yearlyTarget: act.monthlyTarget * 12,
                        yearlyAchieved: 0,
                        proRataTarget: 0
                    };
                }

                // Accumulate Pro-Rata Target ONLY for approved months (for Scoring)
                if (isApproved) {
                    categoriesRaw[act.category].details[act.id].proRataTarget += act.monthlyTarget;

                    const combinedMonthProgress = { ...mProgress };
                    if (monthlyReports) {
                        Object.entries(monthlyReports).forEach(([day, data]: [string, unknown]) => {
                            combinedMonthProgress[day] = { ...(combinedMonthProgress[day] || {}), ...(data as Record<string, boolean>) };
                        });
                    }

                    const achievedInMonth = Object.values(combinedMonthProgress).reduce((count: number, daily: unknown) => {
                        const val = (daily as Record<string, boolean | string>)[act.id];
        ...
        const finalCategories = Object.values(categoriesRaw).map(cat => {
            const details = Object.values(cat.details).map(d => {
                // Calculation:
                // We use Pro-Rata for the "Score" so the Grade is fair based on approved months only.
                const scoringDenominator = d.proRataTarget > 0 ? d.proRataTarget : 1;
                const scorePercentage = d.proRataTarget > 0
                    ? Math.min(100, Math.round((d.yearlyAchieved / scoringDenominator) * 100))
                    // If no months approved yet, default to 100 (Neutral start) or 0? 
                    // To avoid "0.00" confusion at start of year, 100 is safer if no data, 
                    // BUT if Jan is strictly 0 achievement it should be 0.
                    // If proRataTarget is 0, it means NO approved months.
                    : 0; // Default 0 if no approved data at all.

                return {
                    title: d.title,
                    target: d.yearlyTarget, // Show 360
                    achieved: d.yearlyAchieved, // Show 30
                    percentage: scorePercentage // Used for score calculation (Quality)
                };
            });

            // Avoid division by zero if details is empty (unlikely)
            if (details.length === 0) return { name: cat.name, details, score: 0, grade: 'E', bobot: 0 };

            const totalPercentage = details.reduce((sum, d) => sum + d.percentage, 0);
            const averageScore = Math.round(totalPercentage / details.length);
    ...
    const hospital = useMemo(() => {
        // Gabungkan pemeriksaan hospitalId (frontend) dan hospital_id (backend/Database)
        const targetId = employee.hospitalId || (employee as unknown as { hospital_id?: string }).hospital_id;

        if (!hospitals || hospitals.length === 0 || !targetId) return null;

        const hospitalIdDb = (employee as unknown as { hospital_id?: string }).hospital_id;
    ...
    const enrichedProgress = useMemo(() => {
        ...
        const monthlyReports = (employee as unknown as { _monthlyReportsDataCache?: Record<string, any> })._monthlyReportsDataCache?.[monthKey] || {};
    ...
    const yearlyPerformanceData = useMemo(() => {
        ...
        const maxMonthIdx = 12; // Always 12 months context for "Annual"

        // Generate list semu Januari - Desember
        const monthsToCalculate = Array.from({ length: maxMonthIdx }, (_, i) =>
            `${year}-${(i + 1).toString().padStart(2, '0')}`
        );
        ...
        monthsToCalculate.forEach((mKey: string) => {
            ...
            const monthlyReports = (employee as unknown as { _monthlyReportsDataCache?: Record<string, any> })._monthlyReportsDataCache?.[mKey] || {};
            ...
            dailyActivitiesConfig.forEach(act => {
                ...
                if (!categoriesRaw[act.category].details[act.id]) {
                    categoriesRaw[act.category].details[act.id] = {
                        title: act.title,
                        yearlyTarget: act.monthlyTarget * 12, // 🔥 FIX: Target Setahun (Fixed 12 bulan)
                        yearlyAchieved: 0,
                        proRataTarget: 0
                    };
                }
                ...
                if (isApproved) {
                    ...
                    // Hitung capaian di bulan ini (hanya jika approved)
                    const combinedMonthProgress = { ...mProgress };
                    if (monthlyReports) {
                        Object.entries(monthlyReports).forEach(([day, data]: [string, unknown]) => {
                            combinedMonthProgress[day] = { ...(combinedMonthProgress[day] || {}), ...(data as Record<string, boolean>) };
    ...
    const performanceData = useMemo(() => {
        ...
        dailyActivitiesConfig.forEach(activity => {
            ...
            const achieved = Object.values(monthProgress).reduce((dayCount: number, dailyProgress: Record<string, boolean>) => {
                const val = (dailyProgress as Record<string, boolean | string>)[activity.id];
    ...
    const availableMonths = useMemo(() => {
        ...
        const activatedMonths = employee.activatedMonths || (employee as unknown as { activated_months?: string[] }).activated_months || [];

        // Merge and deduplicate
        const allUniqueMonths = Array.from(new Set([...activityMonths, ...activatedMonths]))
            .filter(m => typeof m === 'string' && /^\d{4}-\d{2}$/.test(m));

        const activatedMonthsDb = (employee as unknown as { activated_months?: string[] }).activated_months;
    ...
    if (viewingChecklistFor) {
        ...
        const isMentorApproved = !!sub && sub.status !== 'pending_mentor' && !(sub.status && sub.status.startsWith('rejected_mentor'));
