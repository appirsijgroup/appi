...
const ChartSection = React.memo(({ title, data, color }: { title: string, data: { code: string; count: number; percentage: number }[], color: string }) => (
...
export const QuranCompetencySummaryDashboard: React.FC<QuranCompetencySummaryDashboardProps> = ({
    ...
    const aggregateData = useMemo(() => {
        ...
        filteredMentees.forEach(m => {
            if (m.quranCompetency) {
                ...
                if (comp.readingLevelId) stats.R[comp.readingLevelId] = (stats.R[comp.readingLevelId] || 0) + 1;
                if (comp.tajwidLevelId) stats.T[comp.tajwidLevelId] = (stats.T[comp.tajwidLevelId] || 0) + 1;
                if (comp.memorizationLevelId) stats.H[comp.memorizationLevelId] = (stats.H[comp.memorizationLevelId] || 0) + 1;
                if (comp.understandingLevelId) stats.P[comp.understandingLevelId] = (stats.P[comp.understandingLevelId] || 0) + 1;
            }
        });
    ...
    const TabButton = ({ tab, icon: Icon, label }: { tab: TabView, icon: React.ElementType, label: string }) => (
        <button
                                    ...
                                    {paginatedMentees.map((m) => (
                                        <tr key={m.id} className="hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-center whitespace-nowrap">
                                                <LevelBadge
                                                    code={m.quranCompetency?.readingLevelId || 'R0'}
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <LevelBadge
                                                    code={m.quranCompetency?.tajwidLevelId || 'T0'}
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <LevelBadge
                                                    code={m.quranCompetency?.memorizationLevelId || 'H0'}
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <LevelBadge
                                                    code={m.quranCompetency?.understandingLevelId || 'P0'}
...
QuranCompetencySummaryDashboard.displayName = 'QuranCompetencySummaryDashboard';

export default QuranCompetencySummaryDashboard;
