...
import Bookmarks from '@/components/features/bookmarks/Bookmarks';
...
const ReportReadingModal: React.FC<{
    ...
    return createPortal(
        <div className="fixed inset-0 bg-indigo-950/80 backdrop-blur-md flex items-center justify-center p-4 z-60 animate-fade-in">
            <div className="bg-indigo-900 border border-white/10 rounded-4xl shadow-2xl p-8 w-full max-w-md text-center animate-pop-in">
                <h3 className="text-2xl font-black text-white mb-3 tracking-tight">Lapor Selesai Membaca</h3>
...
export const Alquran: React.FC<AlquranProps> = ({
    ...
    if (isLoadingDetail || selectedSurah) {
        return (
            <div className="bg-indigo-950/40 backdrop-blur-md p-4 sm:p-8 rounded-4xl shadow-2xl border border-white/10 animate-view-change">
                ...
                {selectedSurah && (
                    <>
                        <div className="space-y-4">
                            <div className="text-center p-10 bg-linear-to-br from-indigo-900/40 to-indigo-950/40 rounded-3xl border border-white/10 shadow-xl overflow-hidden relative">
                                <div className="absolute -bottom-10 -right-10 text-[10rem] font-black text-white/5 italic">{selectedSurah.nomor}</div>
                                <p className="font-serif text-6xl text-white tracking-widest drop-shadow-md relative z-10">{selectedSurah.nama}</p>
                                <h2 className="text-4xl font-black text-teal-400 mt-4 relative z-10">{selectedSurah.namaLatin}</h2>
                                <p className="text-blue-100/40 text-lg font-medium mt-2 relative z-10">{selectedSurah.arti}</p>
                                <div className="mt-6 flex items-center justify-center gap-4 relative z-10">
                                    <div className="h-px w-10 bg-white/10" />
                                    <p className="text-xs font-black text-white/20 uppercase tracking-[0.3em]">{selectedSurah.tempatTurun} • {selectedSurah.jumlahAyat} AYAT</p>
                                    <div className="h-px w-10 bg-white/10" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                {selectedSurah.ayat.slice(0, visibleAyahCount).map((ayah) => (
                                    <div key={ayah.nomorAyat} ref={(el) => { ayahRefs.current[ayah.nomorAyat] = el; }} className="bg-indigo-950/40 backdrop-blur-sm p-6 sm:p-10 rounded-4xl border border-white/5 hover:border-white/20 transition-all duration-500 shadow-lg">
