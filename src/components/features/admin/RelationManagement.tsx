                    ...
                    <tbody>
                        {paginatedUsers.map(user => (
                            <tr key={user.id} className="border-b border-gray-700 hover:bg-white/5">
                                            ...
                                            onChange={async (checked) => {
                                                ...
                                                if (checked && !user.canBeManager) {
                                                    await createNotification({
                                                        userId: user.id, type: 'account_role_changed', title: 'Penugasan Sebagai Manajer', message: 'Selamat! Anda telah ditugaskan sebagai Manajer.', linkTo: { view: 'assignment_letter', params: { roleName: 'Manajer', assignmentType: 'designation' } }
                                                    });
                                                } else if (!checked && user.canBeManager) {
                                                    await createNotification({
                                                        userId: user.id, type: 'account_role_changed', title: 'Pencabutan Penugasan Manajer', message: 'Penugasan Anda sebagai Manajer telah berakhir.', linkTo: { view: 'assignment_letter', params: { roleName: 'Manajer', assignmentType: 'revocation' } }
                                                    });
                                                }
                                                return result;
                                            }}
                                        ...
                                        <ToggleSwitch
                                            ...
                                            onChange={async (checked) => {
                                                ...
                                                if (checked && !user.canBeSupervisor) {
                                                    await createNotification({
                                                        userId: user.id, type: 'account_role_changed', title: 'Penugasan Sebagai Supervisor', message: 'Selamat! Anda telah ditugaskan sebagai Supervisor.', linkTo: { view: 'assignment_letter', params: { roleName: 'Supervisor', assignmentType: 'designation' } }
                                                    });
                                                } else if (!checked && user.canBeSupervisor) {
                                                    await createNotification({
                                                        userId: user.id, type: 'account_role_changed', title: 'Pencabutan Penugasan Supervisor', message: 'Penugasan Anda sebagai Supervisor telah berakhir.', linkTo: { view: 'assignment_letter', params: { roleName: 'Supervisor', assignmentType: 'revocation' } }
                                                    });
                                                }
                                                return result;
                                            }}
                                ...
                                <td className="px-2 py-3 border-r border-gray-700 min-w-[160px]">
                        ...
                        {sortedUsers.length === 0 && (
                            <tr>
                                <td colSpan={15} className="text-center p-8 text-blue-200">
                    ...
                    <div className="bg-gray-800 rounded-2xl shadow-2xl p-6 w-full max-w-lg border border-white/20 animate-pop-in max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <EmployeeSearchableInput
                                    allUsers={potentialSupervisors.filter(u => u.id !== editAssignmentModal.id)}
