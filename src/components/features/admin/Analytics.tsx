...
import { SearchIcon, ExcelIcon, PdfIcon, ChartBarIcon, PengaturanIcon as SettingsAltIcon, UsersIcon, ShieldCheckIcon } from '@/components/ui/Icons';
import SimplePagination from '@/components/ui/SimplePagination';
import { generateOfficialPdf, type ReportSection, type TableConfig } from '@/components/ui/ReportGenerator';
...
const ChartCard: React.FC<ChartCardProps> = ({ title, children, minWidth }) => {
    ...
    return (
        <div className="bg-indigo-950/20 backdrop-blur-sm p-3 md:p-4 rounded-xl border border-white/10 transition-all">
...
const HospitalPerformanceComparison: React.FC<{
    ...
    return (
        <div className="bg-black/20 p-4 md:p-6 rounded-xl md:rounded-2xl border border-white/10 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row flex-wrap md:items-center justify-between gap-4 mb-6">
            ...
            {isLoading ? (
                ...
                <ChartCard title={<div className="flex items-center gap-2">{currentConfig.icon}<span>{currentConfig.label}</span></div>}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData} margin={{ top: 25, right: 30, left: 0, bottom: 5 }}>
                            ...
                            <Bar dataKey="value" name="Capaian" radius={[4, 4, 0, 0]} isAnimationActive={false}>
                                <LabelList dataKey="value" position="top" fill={currentConfig.color} fontSize={12} fontWeight="bold" formatter={(val: any) => `${val}%`} />
...
const ActivationReport: React.FC<{
    ...
    useEffect(() => { setCurrentPage(1); }, [hospitalFilter]);

    const totalPages = Math.ceil(stats.hospitalBreakdown.length / itemsPerPage);
    const paginatedData = stats.hospitalBreakdown.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-linear-to-br from-blue-900/40 to-blue-800/20 p-5 rounded-xl border border-blue-500/20 shadow-lg relative overflow-hidden group">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 text-xs text-blue-200/60"><span className="bg-blue-500/20 px-2 py-0.5 rounded text-blue-300 font-medium">Terdaftar</span></div>
                    </div>
                </div>
                ...
                <div className="bg-linear-to-br from-teal-900/40 to-teal-800/20 p-5 rounded-xl border border-teal-500/20 shadow-lg relative overflow-hidden group">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex justify-between text-xs text-teal-200/60"><span>Rate: {stats.activationRate}%</span><span>Bulan Ini</span></div>
                    </div>
                </div>
                ...
                <div className="bg-linear-to-br from-purple-900/40 to-purple-800/20 p-5 rounded-xl border border-purple-500/20 shadow-lg relative overflow-hidden group">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 text-xs text-purple-200/60"><span className="bg-purple-500/20 px-2 py-0.5 rounded text-purple-300 font-medium">Aktif</span></div>
                    </div>
                </div>
                ...
                <div className="bg-linear-to-br from-orange-900/40 to-orange-800/20 p-5 rounded-xl border border-orange-500/20 shadow-lg relative overflow-hidden group">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex justify-end text-xs text-orange-200/60"><span>Bulan Ini</span></div>
                    </div>
                </div>
            </div>
            ...
            {stats.quranStats && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
                    <div className="bg-linear-to-br from-emerald-900/40 to-emerald-800/20 p-5 rounded-xl border border-emerald-500/20 shadow-lg relative overflow-hidden group">
                        <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><BookOpen className="w-16 h-16 text-emerald-300" /></div>
                        <div className="flex flex-col h-full justify-between">
                            <div>
                                <span className="text-emerald-300 text-sm font-medium uppercase tracking-wider flex items-center gap-2"><BookOpen className="w-4 h-4" /> Indeks Literasi Qur'an</span>
                                <div className="mt-2 flex items-baseline gap-2"><span className="text-4xl font-black text-white">{stats.quranStats.indexScore}</span><span className="text-sm text-emerald-200/60">/ 100</span></div>
                                <div className="mt-1 text-xs text-emerald-200/60">Rata-rata Kualitas Bacaan</div>
                            </div>
                            <div className="mt-4 w-full bg-black/30 rounded-full h-1.5 overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-1000 ${stats.quranStats.indexScore > 80 ? 'bg-emerald-500' : stats.quranStats.indexScore > 60 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${stats.quranStats.indexScore}%` }}></div>
                            </div>
                            <div className="mt-2 text-[10px] text-emerald-300/80 font-bold text-right">{stats.quranStats.indexScore > 80 ? 'Sangat Baik (A)' : stats.quranStats.indexScore > 60 ? 'Cukup (B)' : 'Perlu Perbaikan (C)'}</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-linear-to-br from-rose-900/40 to-rose-800/20 p-5 rounded-xl border border-rose-500/20 shadow-lg relative overflow-hidden group">
                        <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><AlertCircle className="w-16 h-16 text-rose-300" /></div>
                        <div className="flex flex-col h-full justify-between">
                            <div>
                                <span className="text-rose-300 text-sm font-medium uppercase tracking-wider flex items-center gap-2"><AlertCircle className="w-4 h-4" /> Perlu Pembinaan</span>
                                <div className="mt-2 flex items-baseline gap-2"><span className="text-4xl font-black text-white">{stats.quranStats.basicCount}</span><span className="text-sm text-rose-200/60">Orang</span></div>
                                <div className="mt-1 text-xs text-rose-200/60">Level Dasar (Belum Lancar)</div>
                            </div>
                            <div className="mt-4 w-full bg-black/30 rounded-full h-1.5 overflow-hidden">
                                <div className="bg-rose-500 h-full rounded-full transition-all duration-1000" style={{ width: `${stats.quranStats.basicRate}%` }}></div>
                            </div>
                            <div className="mt-2 text-[10px] text-rose-300/80 font-bold text-right">{stats.quranStats.basicRate}% dari Total</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-linear-to-br from-cyan-900/40 to-cyan-800/20 p-5 rounded-xl border border-cyan-500/20 shadow-lg relative overflow-hidden group">
                        <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Star className="w-16 h-16 text-cyan-300" /></div>
                        <div className="flex flex-col h-full justify-between">
                            <div>
                                <span className="text-cyan-300 text-sm font-medium uppercase tracking-wider flex items-center gap-2"><Star className="w-4 h-4" /> Kompeten / Lancar</span>
                                <div className="mt-2 flex items-baseline gap-2"><span className="text-4xl font-black text-white">{stats.quranStats.competentCount}</span><span className="text-sm text-cyan-200/60">Orang</span></div>
                                <div className="mt-1 text-xs text-cyan-200/60">Bacaan Sudah Standar</div>
                            </div>
                            <div className="mt-4 w-full bg-black/30 rounded-full h-1.5 overflow-hidden">
                            ...
                            <div className="mt-2 text-[10px] text-cyan-300/80 font-bold text-right">{stats.quranStats.competentRate}% dari Total</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-linear-to-br from-violet-900/40 to-violet-800/20 p-5 rounded-xl border border-violet-500/20 shadow-lg relative overflow-hidden group">
                        <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Crown className="w-16 h-16 text-violet-300" /></div>
                        <div className="flex flex-col h-full justify-between">
                            <div>
                                <span className="text-violet-300 text-sm font-medium uppercase tracking-wider flex items-center gap-2"><Crown className="w-4 h-4" /> Mahir / Hafidz</span>
                                <div className="mt-2 flex items-baseline gap-2"><span className="text-4xl font-black text-white">{stats.quranStats.advancedCount}</span><span className="text-sm text-violet-200/60">Orang</span></div>
                                <div className="mt-1 text-xs text-violet-200/60">Role Model & Calon Mentor</div>
                            </div>
                            <div className="mt-4 w-full bg-black/30 rounded-full h-1.5 overflow-hidden">
                            ...
                            <div className="mt-2 text-[10px] text-violet-300/80 font-bold text-right">{stats.quranStats.advancedRate}% dari Total</div>
                        </div>
                    </div>
                </div>
            )}

            <div className="bg-black/20 rounded-xl border border-white/10 overflow-hidden mt-6">
                <div className="p-4 border-b border-white/10 bg-white/5 flex items-center gap-2">
                    ...
                    <h3 className="text-white font-bold text-sm">{isGlobal ? 'Komparasi Kinerja Unit' : 'Komparasi Kinerja Antar Unit/Bagian'}</h3>
                </div>
                    ...
                    <table className="w-full text-sm text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {paginatedData.length === 0 ? (
                                <tr><td colSpan={5} className="p-8 text-center text-gray-500 italic bg-white/5">Belum ada data unit yang tersedia untuk periode ini.</td></tr>
                            ) : (
                                paginatedData.map((h: any) => {
                                    ...
                                    return (
                                        <tr key={h.id} className="hover:bg-white/5 transition-colors group">
                                            <td className="p-4"><div className="flex flex-col min-w-0"><span className="text-white font-bold group-hover:text-teal-400 transition-colors truncate">{h.brand}</span>{isGlobal && <span className="text-gray-400 text-xs italic truncate" title={h.name}>{h.name}</span>}</div></td>
                                            <td className="p-4 text-center text-white">{h.total}</td>
                                            <td className="p-4"><div className="flex flex-col items-center gap-1"><div className="flex items-center gap-2"><span className="text-white font-semibold">{actPercent}%</span><span className="text-gray-500 text-[10px]">({h.activated}/{h.total})</span></div><div className="w-24 bg-black/40 rounded-full h-1.5 overflow-hidden"><div className="bg-teal-500 h-full rounded-full" style={{ width: `${actPercent}%` }}></div></div></div></td>
                                            <td className="p-4"><div className="flex flex-col items-center gap-1"><div className="flex items-center gap-2"><span className="text-white font-semibold">{compPercent}%</span><span className="text-gray-500 text-[10px]">({h.compliance}/{h.total})</span></div><div className="w-24 bg-black/40 rounded-full h-1.5 overflow-hidden"><div className="bg-amber-500 h-full rounded-full" style={{ width: `${compPercent}%` }}></div></div></div></td>
                                            <td className="p-4 text-center"><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${compPercent > 80 ? 'bg-green-500/20 text-green-400 border border-green-500/30' : compPercent > 50 ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>{compPercent > 80 ? 'TERBAIK' : compPercent > 50 ? 'STABIL' : 'RENDAH'}</span></td>
                                        </tr>
                                    );
                                })
                            )}
                ...
                {stats.hospitalBreakdown.length > 0 && (
                    <SimplePagination currentPage={currentPage} totalPages={totalPages} totalCount={stats.hospitalBreakdown.length} itemsPerPage={itemsPerPage} onPageChange={setCurrentPage} />
                )}
            </div>
        </div>
    );
...
const MutabaahPerformanceReport: React.FC<{
    ...
    const [isClient, setIsClient] = useState(false);
    const [isLoading, setIsLoading] = useState(true);
    const [performanceData, setPerformanceData] = useState<{ performanceByCategory: any[]; groupedPerformanceByActivity: Record<string, any[]>; employeeCount: number; hospitalComparison: any[]; }>({ performanceByCategory: [], groupedPerformanceByActivity: {}, employeeCount: 0, hospitalComparison: [] });

    useEffect(() => { setIsClient(true); }, []);
    const [selectedUserIdFilter, setSelectedUserIdFilter] = useState<string | undefined>(undefined);
    useEffect(() => { setUnitFilter('all'); setBagianFilter('all'); setKategoriFilter('all'); setProfesiFilter('all'); setSelectedUserIdFilter(undefined); }, [hospitalFilter]);
    ...
    useEffect(() => {
        const fetchPerformance = async () => {
            ...
            try {
                const month = (currentMonth.getMonth() + 1).toString().padStart(2, '0');
                const year = currentMonth.getFullYear().toString();
                const params = new URLSearchParams({ month, year, unit: unitFilter, bagian: bagianFilter, professionCategory: kategoriFilter, profession: profesiFilter, hospitalId: hospitalFilter, employeeId: selectedUserIdFilter || 'all' });
                const response = await fetch(`/api/analytics/performance?${params.toString()}`);
                if (response.ok) { setPerformanceData(await response.json()); }
            } catch (error) { console.error('Failed to fetch performance analytics:', error); } finally { setIsLoading(false); }
        };
        fetchPerformance();
    }, [currentMonth, unitFilter, bagianFilter, kategoriFilter, profesiFilter, selectedUserIdFilter, hospitalFilter]);

    const filterOptions = useMemo(() => {
        const units = new Set<string>(); const bagians = new Set<string>(); const profesi = new Set<string>();
        const realEmployees = allUsers.filter(u => u && u.id && !isAdministrativeAccount(u.id) && !isAnyAdmin(u) && (!hospitalFilter || hospitalFilter === 'all' || u.hospitalId?.toLowerCase() === hospitalFilter.toLowerCase()));
        realEmployees.forEach(user => { if (user.unit) units.add(user.unit); if (user.bagian) bagians.add(user.bagian); if (user.profession) profesi.add(user.profession); });
        return { units: Array.from(units).sort(), bagians: Array.from(bagians).sort(), profesi: Array.from(profesi).sort() };
    }, [allUsers, hospitalFilter]);

    const { performanceByCategory, groupedPerformanceByActivity, employeeCount } = performanceData;

    return (
        <div className="bg-indigo-950/20 backdrop-blur-sm p-3 md:p-6 rounded-xl md:rounded-2xl border border-white/10 space-y-6">
            ...
            ` }} />
            <div className="flex flex-col md:flex-row flex-wrap md:items-center justify-between gap-4"><h3 className="text-lg md:text-xl font-bold text-white uppercase tracking-tight">Performa Karyawan</h3></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-3"><EmployeeSearchableInput allUsers={allUsers} value={selectedUserIdFilter} onChange={setSelectedUserIdFilter} placeholder="Cari & Filter Nama Karyawan..." /></div>
                <SelectWrapper icon={<Filter size={16} />}><select value={unitFilter} onChange={e => setUnitFilter(e.target.value)} className={`${SELECT_CLASS} pl-10`}><option value="all" className="bg-slate-900 text-white">Semua Unit</option>{filterOptions.units.map(opt => <option key={opt} value={opt} className="bg-slate-900 text-white">{opt}</option>)}</select></SelectWrapper>
                <SelectWrapper icon={<Filter size={16} />}><select value={bagianFilter} onChange={e => setBagianFilter(e.target.value)} className={`${SELECT_CLASS} pl-10`}><option value="all" className="bg-slate-900 text-white">Semua Bagian</option>{filterOptions.bagians.map(opt => <option key={opt} value={opt} className="bg-slate-900 text-white">{opt}</option>)}</select></SelectWrapper>
                <SelectWrapper icon={<UsersIcon className="w-4 h-4" />}><select value={kategoriFilter} onChange={e => setKategoriFilter(e.target.value as any)} className={`${SELECT_CLASS} pl-10`}><option value="all" className="bg-slate-900 text-white">Semua Kategori Profesi</option><option value="MEDIS" className="bg-slate-900 text-white">MEDIS</option><option value="NON MEDIS" className="bg-slate-900 text-white">NON MEDIS</option></select></SelectWrapper>
                <SelectWrapper icon={<Filter size={16} />}><select value={profesiFilter} onChange={e => setProfesiFilter(e.target.value)} className={`${SELECT_CLASS} pl-10`}><option value="all" className="bg-slate-900 text-white">Semua Profesi</option>{filterOptions.profesi.map(opt => <option key={opt} value={opt} className="bg-slate-900 text-white">{opt}</option>)}</select></SelectWrapper>
            </div>
            {isLoading ? (<div className="flex flex-col items-center justify-center p-20 bg-indigo-950/20 backdrop-blur-sm rounded-xl border border-white/10"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-teal-400"></div></div>) : (
                <div className="space-y-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-700">
                        {performanceByCategory.map((category: any, index: number) => {
                            const colors = [{ bg: 'bg-amber-900/40', border: 'border-amber-500/20', text: 'text-amber-400' }, { bg: 'bg-teal-900/40', border: 'border-teal-500/20', text: 'text-teal-400' }, { bg: 'bg-blue-900/40', border: 'border-blue-500/20', text: 'text-blue-400' }, { bg: 'bg-purple-900/40', border: 'border-purple-500/20', text: 'text-purple-400' }];
                            ...
                            return (
                                <div key={category.name} className={`${style.bg} p-4 rounded-xl border ${style.border} shadow-lg relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300`}>
                                    <div className="flex flex-col">
                                        <span className={`${style.text} text-[10px] md:text-xs font-bold uppercase tracking-wider mb-1 opacity-80 truncate`}>{category.name}</span>
                                        <div className="flex items-baseline gap-1"><span className="text-2xl md:text-3xl font-black text-white">{category.Persentase}%</span><span className="text-[10px] text-gray-400">Rata-rata</span></div>
                                        <div className="w-full bg-black/30 rounded-full h-1.5 mt-2 overflow-hidden"><div className={`h-full rounded-full transition-all duration-1000 ${style.text.replace('text-', 'bg-')}`} style={{ width: `${category.Persentase}%` }}></div></div>
                                    </div>
                                </div>
                            );
                        })}
                    </div>
                    <ChartCard title="Rata-rata Capaian per Kategori" minWidth="700px"><ResponsiveContainer width="100%" height="100%"><BarChart data={performanceByCategory} margin={{ top: 20, right: 20, left: -10, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="#374151" /><XAxis dataKey="name" stroke="#cbd5e1" fontSize={12} /><YAxis stroke="#cbd5e1" allowDecimals={false} domain={[0, 100]} tickFormatter={(tick) => `${tick}%`} /><Bar dataKey="Persentase" isAnimationActive={false}><LabelList dataKey="Persentase" position="top" fill="#e2e8f0" fontSize={12} formatter={(v: any) => `${v}%`} />{performanceByCategory.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Bar></BarChart></ResponsiveContainer></ChartCard>
                    {Object.entries(groupedPerformanceByActivity).map(([category, activities], index) => (
                        <ChartCard key={category} title={`Detail Kategori: ${category}`} minWidth="700px"><ResponsiveContainer width="100%" height="100%"><BarChart data={activities} layout="vertical" margin={{ top: 5, right: 60, left: 10, bottom: 20 }}><CartesianGrid strokeDasharray="3 3" stroke="#374151" /><XAxis type="number" stroke="#94a3b8" domain={[0, 100]} tickFormatter={(tick) => `${tick}%`} /><YAxis dataKey="name" type="category" stroke="#94a3b8" width={220} tick={<CustomYAxisTick />} interval={0} /><Bar dataKey="percentage" name="Capaian" barSize={20} fill={COLORS[index % COLORS.length]} isAnimationActive={false}><LabelList dataKey="percentage" position="right" fill="#e2e8f0" fontSize={11} formatter={(v: any) => v > 0 ? `${v}%` : ''} /></Bar></BarChart></ResponsiveContainer></ChartCard>
                    ))}
                </div>
            )}
        </div>
...
const Analytics: React.FC<AnalyticsProps> = ({ allUsersData, dailyActivitiesConfig, onLoadAllData }) => {
    ...
    const canSelectHospital = useMemo(() => {
        ...
        return loggedInEmployee.canBeBPH || loggedInEmployee.functionalRoles?.includes('BPH') || (loggedInEmployee as any).functional_roles?.includes('BPH') || isSuperAdmin(loggedInEmployee) || (loggedInEmployee.managedHospitalIds?.length || 0) > 0;
    }, [loggedInEmployee]);

    const accessibleHospitals = useMemo(() => {
        if (!loggedInEmployee) return [];
        if (loggedInEmployee.canBeBPH || loggedInEmployee.functionalRoles?.includes('BPH') || (loggedInEmployee as any).functional_roles?.includes('BPH') || isSuperAdmin(loggedInEmployee)) return hospitals;
        const allowedIds = [loggedInEmployee.hospitalId, ...(loggedInEmployee.managedHospitalIds || [])].filter(Boolean).map(id => id?.toLowerCase());
    ...
    useEffect(() => {
        if (loggedInEmployee && !isFilterInitialized) {
            if (!(loggedInEmployee.canBeBPH || loggedInEmployee.functionalRoles?.includes('BPH') || (loggedInEmployee as any).functional_roles?.includes('BPH') || isSuperAdmin(loggedInEmployee))) {
                if (loggedInEmployee.hospitalId) setHospitalFilter(loggedInEmployee.hospitalId);
            }
    ...
    const navigateMonth = (direction: 'prev' | 'next') => { setCurrentMonth(prev => { const d = new Date(prev); d.setDate(1); d.setMonth(d.getMonth() + (direction === 'prev' ? -1 : 1)); return d; }); };
    const isNextMonthFuture = () => { const n = new Date(currentMonth); n.setDate(1); n.setMonth(n.getMonth() + 1); return n > new Date(); };

    const [stats, setStats] = useState({ totalEmployees: 0, activatedCount: 0, notActivatedCount: 0, mentorCount: 0, complianceRate: 0, activationRate: 0, hospitalBreakdown: [] as any[], quranStats: null as any });

    useEffect(() => { if (canSelectHospital && onLoadAllData) onLoadAllData(); }, [canSelectHospital, onLoadAllData]);
    ...
    const allUsers = useMemo(() => {
        return Object.values(allUsersData).map(d => d.employee).filter(e => e && e.id && e.isActive !== false && !isAdministrativeAccount(e.id) && e.role !== 'admin' && e.role !== 'super-admin' && (hospitalFilter === 'all' || e.hospitalId?.toLowerCase() === hospitalFilter.toLowerCase()));
    }, [allUsersData, hospitalFilter]);

    const selectedHospitalName = useMemo(() => hospitalFilter === 'all' ? 'Grup RSI (Aliansi)' : hospitals.find(h => h.id === hospitalFilter)?.name || 'Rumah Sakit', [hospitalFilter, hospitals]);
    ...
    return (
        <div className="space-y-8 pb-10">
            ...
            ` }} />
            <div className={`p-1 rounded-2xl border ${isGlobal ? 'bg-amber-900/20 border-amber-500/30' : 'bg-teal-900/20 border-teal-500/30'} backdrop-blur-md`}>
                <div className="flex flex-col md:flex-row flex-wrap md:items-center p-2 gap-4">
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl ${isGlobal ? 'bg-amber-500/10 text-amber-100' : 'bg-teal-500/10 text-teal-100'} grow`}>
                        <div className={`p-2 rounded-lg ${isGlobal ? 'bg-amber-500/20 text-amber-400' : 'bg-teal-500/20 text-teal-400'}`}><LayoutDashboard size={20} /></div>
                        <span className={`text-sm md:text-lg font-black ${isGlobal ? 'text-amber-400' : 'text-teal-400'}`}>{isGlobal ? 'RSIJ GROUP' : selectedHospitalName}</span>
                    </div>
                    <div className="grid grid-cols-2 bg-black/40 p-1 rounded-xl border border-white/10 shrink-0 md:min-w-[340px] md:w-auto w-full">
            ...
            <div className="flex justify-center -mt-6 mb-6">
                <div className="flex items-center bg-black/60 border border-white/10 rounded-2xl p-1 shadow-2xl">
                    <button onClick={() => navigateMonth('prev')} className="p-2 text-white"><ChevronDown className="w-5 h-5 rotate-90" /></button>
                    <div className="min-w-[160px] text-center px-6 border-x border-white/5"><span className="text-[10px] text-teal-400 uppercase block">Periode Laporan</span><span className="font-bold text-sm text-white">{currentMonth.toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}</span></div>
                    <button onClick={() => navigateMonth('next')} disabled={isNextMonthFuture()} className="p-2 text-white disabled:opacity-20"><ChevronDown className="w-5 h-5 -rotate-90" /></button>
                </div>
            </div>

            {isLoadingStats ? (<div className="flex items-center justify-center py-32 bg-indigo-950/20 backdrop-blur-md rounded-3xl border border-white/10"><div className="w-16 h-16 border-4 border-teal-500 border-t-transparent rounded-full animate-spin"></div></div>) : viewMode === 'overview' ? (
                <div className="space-y-8"><ActivationReport allUsers={allUsers} hospitalFilter={hospitalFilter} hospitalName={selectedHospitalName} stats={stats} />{isGlobal && canSeeGlobal && <div className="space-y-8"><GlobalComparisonCharts breakdown={stats.hospitalBreakdown} /><HospitalPerformanceComparison hospitalFilter={hospitalFilter} hospitals={hospitals} currentMonth={currentMonth} /></div>}</div>
            ) : (
                <div className="p-0 md:p-6 bg-transparent md:bg-black/40 rounded-3xl md:border md:border-white/10"><MutabaahPerformanceReport key={`${hospitalFilter}-${viewMode}`} allUsers={allUsers} dailyActivitiesConfig={dailyActivitiesConfig} hospitalFilter={hospitalFilter} hospitals={hospitals} currentMonth={currentMonth} navigateMonth={navigateMonth} isNextMonthFuture={isNextMonthFuture} /></div>
            )}
        </div>
    );
};
