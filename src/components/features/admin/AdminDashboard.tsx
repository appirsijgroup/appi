'use client';
// @ts-nocheck
/* eslint-disable react-hooks/exhaustive-deps -- Dependencies are complex in this large component */
...
import { availableIconsForSunnah } from '@/components/ui/Icons';
import { generateOfficialPdf, type TableConfig, type ReportSection } from '@/components/ui/ReportGenerator';
import PdfPreviewModal from '@/components/ui/PdfPreviewModal';
import ConfirmationModal from '@/components/ui/ConfirmationModal';
import MutabaahReport from '@/components/features/mutabaah/MutabaahReport';
    ...
    const handleProcessImport = () => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                const expectedHeaders = ['RS ID', 'NIP', 'Nama']; // Minimal required headers
                ...
                const getSanitizedCategory = (rawCategory: unknown): 'MEDIS' | 'NON MEDIS' => {
                    if (!rawCategory) return 'NON MEDIS'; // Default fallback
                ...
                const getSanitizedGender = (rawGender: unknown): 'Laki-laki' | 'Perempuan' => {
                    if (!rawGender) return 'Laki-laki'; // Default fallback
                ...
                const usersToProcess: (RawEmployee & { id: string, role?: Role })[] = json.map(row => ({
                    ...
                    unit: row.Unit ? String(row.Unit).trim() : 'Unit Belum Diisi',
                    bagian: row.Bagian ? String(row.Bagian).trim() : 'Bagian Belum Diisi',
                    professionCategory: getSanitizedCategory(row['Kategori Profesi']),
                    profession: row.Profesi ? String(row.Profesi).trim() : '-',
            ...
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6 mb-8 shadow-2xl">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-12 gap-4 items-end pt-2 border-t border-white/5">
                        <div className="md:col-span-4 lg:col-span-6 xl:col-span-3 2xl:col-span-3">
                        ...
                        <div className="md:col-span-8 lg:col-span-6 xl:col-span-5 2xl:col-span-5">
                        ...
                        {reportType === 'prayer' && dateFilterType === 'monthly' && (
                            <div className="md:col-span-6 lg:col-span-6 xl:col-span-4 2xl:col-span-2">
                        ...
                        <div className={`md:col-span-6 ${reportType === 'prayer' && dateFilterType === 'monthly' ? 'lg:col-span-6 xl:col-span-8 2xl:col-span-2' : 'lg:col-span-12 xl:col-span-4 2xl:col-span-4'} flex items-center justify-end gap-3`}>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="mt-1 flex items-center gap-4">
                            {logo ? (
                                <Image src={logo} alt="Logo preview" width={80} height={80} className="h-20 w-20 object-contain rounded-md bg-white p-1 shadow-md" unoptimized />
                            ) : (
                    ...
                    <tbody>
                        {sortedHospitals.map(hospital => (
                            <tr key={hospital.id} className="border-b border-gray-700 hover:bg-white/5">
                                <td className="px-4 py-3 whitespace-nowrap">
                                    {hospital.logo ? (
                                        <Image src={hospital.logo} alt={`${hospital.brand} logo`} width={40} height={40} className="h-10 w-10 object-contain rounded-md bg-white p-0.5" unoptimized />
                                    ) : (
...
const ResetPasswordManagement: React.FC<{ allUsers: Employee[] }> = ({ allUsers }) => {
    ...
    const handleReset = async () => {
        ...
        try {
            ...
            if (result.success) {
                setMessage({ text: `Password untuk ${selectedUser?.name || selectedUserId} berhasil direset.`, type: 'success' });
                setNewPassword('');
                setSelectedUserId(undefined);
            } else {
                setMessage({ text: result.error || 'Gagal meriset password.', type: 'error' });
            }
        } catch (error) {
    ...
    return (
        <div className="flex justify-center p-4">
            <div className="w-full max-w-xl bg-gray-800/50 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden">
                <div className="p-8 space-y-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-white uppercase tracking-tight">Reset Password Pegawai</h3>
                            <p className="text-gray-400 text-sm">Gunakan menu ini jika pegawai lupa password.</p>
                        </div>
                    </div>

                    <div className="space-y-6">
                        <div>
                            <label className="block text-sm font-bold text-teal-400 uppercase tracking-widest mb-2 pl-1">Cari Pegawai (Nama/NIP)</label>
                        ...
                        {selectedUser && (
                            <div className="p-4 bg-teal-500/10 border border-teal-500/20 rounded-2xl animate-in fade-in slide-in-from-top-2">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-teal-600/30 flex items-center justify-center text-teal-300 font-bold border border-teal-500/20 text-lg shadow-inner">
                                        {selectedUser.name.charAt(0)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-white font-bold text-lg truncate tracking-tight">{selectedUser.name}</p>
                                        <p className="text-gray-400 text-xs uppercase tracking-wider font-mono font-bold">{selectedUser.id} • {selectedUser.unit}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div>
                            <label className="block text-sm font-bold text-teal-400 uppercase tracking-widest mb-2 pl-1">Password Baru</label>
                            <div className="relative group">
                                <input
                                    ...
                                    className="w-full bg-white/10 border border-white/30 rounded-lg p-3 pr-12 text-white focus:ring-2 focus:ring-teal-400 focus:outline-none transition-all"
                                />
                                <button
                                    type="button"
                                    onClick={() => setShowPassword(!showPassword)}
                                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-teal-400 transition-colors bg-white/5 rounded-md"
                    ...
                    {message && (
                        <div className={`p-4 rounded-xl text-sm font-bold flex items-center gap-3 animate-in zoom-in-95 duration-300 ${message.type === 'success'
                    ...
                    <div className="pt-4 flex justify-center">
                        <button
                            onClick={handleReset}
                            disabled={!selectedUserId || !newPassword || isSubmitting}
                            className="px-8 py-3 rounded-xl bg-teal-500 hover:bg-teal-400 disabled:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed text-gray-900 font-bold text-sm transition-all active:scale-[0.98] shadow-lg shadow-teal-500/20 flex items-center justify-center gap-2"
                        >
                            {isSubmitting ? (
                                <>
...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
    ...
    const [activeView, setActiveView] = useState<AdminView>(() => {
        if (typeof window !== 'undefined') {
            ...
            if (savedView && ['manajemen-pengguna', 'manajemen-konten', 'reports', 'manajemen-rs', 'manajemen-admin', 'reset-password'].includes(savedView)) {
...
`}</style>
                        ...
                        {isAnyAdmin(loggedInEmployee) && (
                            <TabButton active={activeView === 'reset-password'} onClick={() => setActiveView('reset-password')} label="Reset Password" icon={Lock} />
                        )}
                    </div>
            ...
            <div className="bg-black/10 md:bg-black/20 p-3 md:p-6 rounded-xl border border-white/10 shadow-inner">
                ...
                {activeView === 'reset-password' && isAnyAdmin(loggedInEmployee) && (
                    <ResetPasswordManagement allUsers={allUsers} />
                )}
            </div>
