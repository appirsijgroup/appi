...
import AktivitasSaya from '@/components/features/mutabaah/AktivitasSaya';
...
const AktivitasSayaContainer: React.FC<AktivitasSayaContainerProps> = ({ initialTab }) => {
    ...
    const { allUsersData, setAllUsersData, loadDetailedEmployeeData, loadAllEmployees, activityStatsRefreshCounter } = useEmployeeStore();
    ...
    const { activities, teamAttendanceSessions, teamAttendanceRecords, addActivity, addTeamAttendanceSessions, createTeamAttendanceRecord, updateTeamAttendanceSessionData, deleteTeamAttendanceSession, loadTeamAttendanceSessions, loadActivities } = useActivityStore();
    const { monthlyReportSubmissions, tadarusSessions, tadarusRequests, missedPrayerRequests, menteeTargets, addOrUpdateMonthlyReportSubmission, addTadarusSessions, updateTadarusSession, deleteTadarusSession, addOrUpdateTadarusRequest, addOrUpdateMissedPrayerRequest, addMenteeTarget, updateMenteeTarget, deleteMenteeTarget, loadTadarusRequests, loadMissedPrayerRequests, loadMonthlyReportSubmissions } = useGuidanceStore();
    ...
    const syncOldTeamAttendanceData = useCallback(async () => {
        ...
        try {
            ...
            for (const record of teamAttendanceRecords) {
                ...
                // Simpan ke allUsersData dan Database saja
    ...
    const handleLogManualActivity = useCallback(async (activityId: string, date: string): Promise<boolean> => {
        ...
        if (isDateValidForMutabaahUpdate(date, loggedInEmployee)) {
            ...
            try {
                ...
                // Update Employee Store (allUsersData)
                setAllUsersData((prev: Record<string, { employee: Employee; attendance: Attendance; history: HistoryRecord }>) => {
    ...
    }, [loggedInEmployee, allUsersData, isDateValidForMutabaahUpdate, addToast, setAllUsersData, setLoggedInEmployee]);
    ...
    const handleLogBookReading = useCallback(async (bookTitle: string, pagesRead: string, dateCompleted: string) => {
        ...
        try {
            ...
            if (activityIdToUpdate) {
                ...
                // Calculate optimistic monthly activities update
                const dateObj = new Date(dateCompleted + 'T12:00:00Z');
                const mKey = `${dateObj.getFullYear()}-${(dateObj.getMonth() + 1).toString().padStart(2, '0')}`;
                const dKey = dateObj.getDate().toString().padStart(2, '0');

                const monthlyActivities = { ...(loggedInEmployee.monthlyActivities || {}) };
                const mData = { ...(monthlyActivities[mKey] || {}) };
                const dData = { ...(mData[dKey] || {}) };
                dData[activityIdToUpdate] = true;
                mData[dKey] = dData;
                monthlyActivities[mKey] = mData;

                const newMonthlyActivities = monthlyActivities;
                ...
                // 🔥 CRITICAL: Update Auth Store too
                setLoggedInEmployee({
                    ...loggedInEmployee,
                    readingHistory: updatedHistory,
                    monthlyActivities: newMonthlyActivities
            ...
            if (updatedSubmission) {
                ...
                if (submission) {
                    ...
                    createNotification({
                        ...
                        message: message,
                        linkTo: `/aktifitas-saya?reportId=${submissionId}`,
    ...
    const aktivitasSayaComponent = loggedInEmployee ? (
        <AktivitasSaya
            ...
            initialTab={((searchParams?.get('tab') as string) || initialTab) as "aktivitas-pribadi" | "riwayat-bacaan" | "panel-mentor" | "persetujuan" | undefined}
            tadarusRequests={tadarusRequests}
            tadarusSessions={tadarusSessions}
            missedPrayerRequests={missedPrayerRequests}
            onUpdateProfile={handleUpdateProfile}
            onReviewReport={handleReviewReport}
            onCreateTadarusSession={(data: Omit<TadarusSession, 'id' | 'createdAt' | 'presentMenteeIds'>) => {
                addTadarusSessions([data as unknown as TadarusSession]);
            }}
            onUpdateTadarusSession={(sessionId: string, updates: Record<string, unknown>) => {
                updateTadarusSession(sessionId, updates as Partial<TadarusSession>);
            }}
            onDeleteTadarusSession={deleteTadarusSession}
            onReviewTadarusRequest={async (requestId: string, status: string) => {
                const req = tadarusRequests.find((r: TadarusRequest) => r.id === requestId);
                if (!req) return;
                ...
                try {
                    ...
                    const updatedRequest: TadarusRequest = {
                        ...req,
                        status: status as TadarusRequest['status'],
                        reviewedAt: Date.now()
                    };
            ...
            onReviewMissedPrayerRequest={async (requestId: string, status: string, mentorNotes?: string) => {
                const req = missedPrayerRequests.find((r: MissedPrayerRequest) => r.id === requestId);
                if (!req) return;
                ...
                try {
                    ...
                    const updatedRequest: MissedPrayerRequest = {
                        ...req,
                        status: status as MissedPrayerRequest['status'],
                        reviewedAt: Date.now(),
