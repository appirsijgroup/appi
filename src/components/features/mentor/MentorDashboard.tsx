...
import { type Employee, type MonthlyReportSubmission, type TadarusSession, type TadarusRequest, type MissedPrayerRequest, type MenteeTarget, type Attendance, type DailyActivity, type HistoryRecord, type QuranReadingHistory } from '@/types';
import {
    ArrowLeft,
    CheckSquare,
    Square,
...
import { generateOfficialPdf, ReportSection, TableConfig } from '@/components/ui/ReportGenerator';
import { useUIStore } from '@/store/store';
import UniversalPersetujuan from '@/components/features/mentor/Persetujuan';
import { isAnyAdmin, isSuperAdmin } from '@/lib/rolePermissions';
import SimplePagination from '@/components/ui/SimplePagination';
import { QuranCompetencyAssessment } from '@/components/features/quran/QuranCompetencyAssessment';
import { QuranCompetencySummaryDashboard } from '@/components/features/quran/QuranCompetencySummaryDashboard';
...
interface MentorDashboardProps {
    employee: Employee;
    allUsersData: Record<string, { employee: Employee; attendance: Attendance; history: HistoryRecord; }>;
...
const MenteeDetailProgressView: React.FC<{
    ...
    allUsersData?: Record<string, { employee: Employee; attendance: Attendance; history: Record<string, unknown>; }>;
    loadDetailedEmployeeData?: (employeeId: string, monthOrForce?: number | boolean, year?: number, force?: boolean) => Promise<void>;
    dailyActivitiesConfig: DailyActivity[];
    ...
    useEffect(() => {
        if (selectedMenteeId && loadDetailedEmployeeData) {
            // Only show loading if we don't have activities for this month yet
            const hasData = selectedMentee?.monthlyActivities?.[activeMonthKey];
            if (!hasData) {
            ...
            loadDetailedEmployeeData(selectedMenteeId, m, y)
                .finally(() => setIsDetailLoading(false));
        }
    }, [selectedMenteeId, activeMonthKey, loadDetailedEmployeeData, activeDate, selectedMentee?.monthlyActivities]);
    // Note: selectedMentee is not in deps to avoid loop, but it changes when load finishes anyway.
    ...
    const enrichedProgress = useMemo(() => {
        ...
        const monthlyReports = (selectedMentee as unknown as Record<string, any>)._monthlyReportsDataCache?.[activeMonthKey] || {};
        Object.entries(monthlyReports).forEach(([dayKey, dayData]) => {
        ...
        if (selectedMentee.readingHistory) {
            selectedMentee.readingHistory.forEach(h => {
                if (h.dateCompleted && h.dateCompleted.startsWith(activeMonthKey)) {
        ...
        if (selectedMentee.quranReadingHistory) {
            (selectedMentee.quranReadingHistory as QuranReadingHistory[]).forEach((h) => {
                const date = h.date;
    ...
    const activityProgressCounts = useMemo(() => {
        ...
        if (selectedMentee.readingHistory) {
            totalReadingCount += selectedMentee.readingHistory.filter(h => h.dateCompleted && h.dateCompleted.startsWith(activeMonthKey)).length;
        }
        if (selectedMentee.quranReadingHistory) {
            (selectedMentee.quranReadingHistory as QuranReadingHistory[]).forEach((h) => {
                const date = h.date;
                if (date && date.startsWith(activeMonthKey)) totalReadingCount++;
            });
        }
        ...
        return dailyActivitiesConfig.reduce((acc, activity) => {
            if (activity.id === 'baca_alquran_buku') {
                ...
                acc[activity.id] = Object.values(enrichedProgress).reduce((dayCount, dailyProgress) => {
                    const progress = dailyProgress as any;
                    // 🔥 Check ID directly first, then check case-insensitive match if needed
                    let isCompleted = progress[activity.id];

                    if (!isCompleted) {
                        const matchingKey = Object.keys(progress).find(k => k.toLowerCase() === activity.id.toLowerCase());
                        if (matchingKey) {
                            isCompleted = progress[matchingKey];
                        }
                    }

                    return dayCount + (isCompleted ? 1 : 0);
                }, 0);
            }
            return acc;
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                ...
                {mentees && mentees.length > 0 && (
                    <div className="w-full md:w-auto grow md:grow-0 flex items-center gap-4">
                        ...
                        <div className="shrink-0 flex items-center justify-between bg-black/20 p-1.5 rounded-full w-auto border border-white/5">
                            <button onClick={() => navigateMonth('prev')} className="px-5 py-2 rounded-full hover:bg-white/10 active:bg-white/20 transition-all font-bold">&larr;</button>
                            <button onClick={() => navigateMonth('next')} disabled={isNextMonthFuture()} className="px-5 py-2 rounded-full hover:bg-white/10 active:bg-white/20 transition-all font-bold disabled:opacity-50">&rarr;</button>
                        </div>
                        <button
            ...
            {selectedMentee && selectedMentee.quranCompetency && (
                <div className="flex flex-wrap gap-4 mb-6 p-4 bg-teal-500/5 border border-teal-500/20 rounded-2xl">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {[
                            { label: 'Baca', code: selectedMentee.quranCompetency.readingLevelId, color: 'text-emerald-400' },
                            { label: 'Tajwid', code: selectedMentee.quranCompetency.tajwidLevelId, color: 'text-blue-400' },
                            { label: 'Hafalan', code: selectedMentee.quranCompetency.memorizationLevelId, color: 'text-purple-400' },
                            { label: 'Adab', code: selectedMentee.quranCompetency.understandingLevelId, color: 'text-amber-400' }
                        ].map(stat => (
            ...
            {selectedMentee ? (
                <>
                    <div className="relative overflow-x-auto rounded-lg border border-white/20">
                        {isDetailLoading && (
                            <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-[2px] z-30 flex items-center justify-center">
                                <div className="flex flex-col items-center gap-2">
                                    <RefreshCw className="w-8 h-8 text-teal-400 animate-spin" />
                                    <span className="text-teal-200 text-sm font-medium">Sinkronisasi data...</span>
                                </div>
                            </div>
                        )}
                                        ...
                                        {(activities as DailyActivity[]).map(activity => (
                                            <tr key={activity.id} className="border-b border-gray-700 hover:bg-white/5">
                                                ...
                                                {Array.from({ length: daysInMonth }, (_, i) => String(i + 1).padStart(2, '0')).map(day => {
                                                    const isChecked = (enrichedProgress[day] as Record<string, boolean>)?.[activity.id] || false;
                                                    return (
                    ...
                    <div className="mt-8 pt-8 border-t border-white/20">
                        ...
                        {chartSummaryData && chartSummaryData.length > 0 ? (
                            <div className="overflow-x-auto pb-4 -mx-2 px-2 md:overflow-x-visible md:mx-0 md:px-0">
                                <div className="min-w-[700px] md:min-w-0 w-full" style={{ height: 600, minHeight: 600 }}>
                                    <ResponsiveContainer width="100%" height={600} minHeight={600} minWidth={0}>
                                        <BarChart
                                            layout="vertical"
                                        ...
                                        >
                                            ...
                                            />
                                            <Bar dataKey="percentage" name="Capaian" isAnimationActive={false} barSize={20} radius={[0, 4, 4, 0]}>
                                                <LabelList dataKey="percentage" position="right" fill="#e2e8f0" fontSize={11} formatter={(value: any) => {
...
interface PersetujuanProps {
    ...
    filteredItems: Array<{ id: string; type: 'report' | 'tadarus' | 'prayer'; menteeName: string; periode: string; submittedAt: number; status: string; originalData?: MonthlyReportSubmission }>; // Unified items for the table
                        ...
                        <tbody>
                            {filteredItems.map(item => (
                                <tr key={`${item.type} -${item.id} `} className="border-b border-gray-700 hover:bg-white/5">
                                    ...
                                    <td className="px-4 py-3 text-center whitespace-nowrap">
                                        {item.type === 'report' ? (
                                            <button onClick={() => item.originalData && onViewReport(item.originalData)} className="px-3 py-1.5 rounded-md font-semibold text-xs bg-blue-600 hover:bg-blue-500 text-white transition-colors">
                                                Lihat Detail
                                            </button>
                                        ) : (
...
export const MentorDashboard: React.FC<MentorDashboardProps> = ({
    ...
    // 🔥 AUTO-LOAD: Pre-load data for all mentees when progress or reading tabs are opened
    useEffect(() => {
        const needsDetailedData = mentorSubView === 'progress' || mentorSubView === 'laporan-bacaan' || mentorSubView === 'quran-competency';
        if (needsDetailedData && mentees.length > 0) {
            const now = new Date();
            const currentMonth = now.getMonth() + 1;
            const currentYear = now.getFullYear();

            // 1. Bulk load activities (Super Fast)
            if (loadTeamProgressBulk) {
                loadTeamProgressBulk(currentMonth, currentYear).catch(console.error);
            }

            // 2. Pre-load reading history & quran competency for UI readiness
            const { loadTeamReadingHistory } = useGuidanceStore.getState();
            loadTeamReadingHistory().catch(console.error);

            // 3. Optional: Background refresh for the currently selected/first mentee
            if (loadDetailedEmployeeData && mentees[0]?.id) {
                loadDetailedEmployeeData(mentees[0].id, currentMonth, currentYear).catch(console.error);
            }
        }
    }, [mentorSubView, mentees, loadTeamProgressBulk, loadDetailedEmployeeData]);
    ...
    const unifiedHistory = useMemo(() => {
        ...
        const tadarus = tadarusRequests.filter(r => r.mentorId === employee.id).map(r => ({
            ...
            periode: `${r.category || 'Tadarus'} (${r.date ? new Date(r.date + 'T12:00:00Z').toLocaleDateString('id-ID', { day: '2-digit', month: 'short' }) : '?'})`,
            submittedAt: r.requestedAt,
            monthKey: r.date ? r.date.substring(0, 7) : '',
            status: r.status,
            originalData: r
        }));
        ...
        const missedPrayers = missedPrayerRequests.filter(r => r.mentorId === employee.id).map(r => ({
            ...
            menteeName: r.menteeName,
            periode: `Terlewat: ${r.prayerName} (${r.date ? new Date(r.date + 'T12:00:00Z').toLocaleDateString('id-ID', { day: '2-digit', month: 'short' }) : '?'})`,
            submittedAt: r.requestedAt,
            monthKey: r.date ? r.date.substring(0, 7) : '',
            status: r.status,
            originalData: r
    ...
    const filteredItems = useMemo(() => {
        return unifiedHistory.filter(item => {
            const statusMatch = statusFilter === 'all' ||
                ...
                (statusFilter === 'rejected' && (item.type === 'report' ? (item.status && item.status.startsWith('rejected_')) : item.status === 'rejected'));
