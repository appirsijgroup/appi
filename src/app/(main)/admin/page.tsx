...
const AdminDashboard = dynamicImport(() => import('@/components/features/admin/AdminDashboard'), {
...
import { getAllEmployees, updateEmployee as updateEmployeeDb, deleteEmployee as deleteEmployeeDb, createEmployee as createEmployeeDb, convertToCamelCase } from '@/services/employeeService';
import { getPaginatedEmployees } from '@/services/employeeServicePaginated';
import { getAllHospitals, createHospital as createHospitalDb, updateHospital as updateHospitalDb, deleteHospital as deleteHospitalDb, toggleHospitalStatus as toggleHospitalStatusDb } from '@/services/hospitalService';
import { validateRoleChange } from '@/lib/rolePermissions';
import bcrypt from 'bcryptjs';

export default function AdminPage() {

    const { loggedInEmployee, isHydrated } = useAuthStore();
    const {
        allUsersData,
        setAllUsersData,
        loadAllEmployees,
    ...
    const handleConfirmMutabaahChange = async () => {
        ...
        // Simpan ke Database (dengan isSuperAdmin=true dan userId)
    ...
    // Load admin data from Database (NOT including employee data - loaded on-demand)
    useEffect(() => {
    ...
    const handleToggleStatus = async (userId: string) => {
        try {
            ...
            // Update in Database
            await updateEmployeeDb(userId, { isActive: newStatus });
    ...
    const handleSetRole = async (userId: string, newRole: Role) => {
        try {
            ...
            // Update in Database
            await updateEmployeeDb(userId, { role: newRole });
            ...
            setAllUsersData((prev) => {
                ...
                if (loggedInEmployee) {
                    // Log audit logic could be implemented here
                }
                return newData;
            });
    ...
    const handleUpdateUser = async (id: string, updates: RawEmployee) => {
        try {
            await updateEmployeeDb(id, updates);
        ...
        for (const user of usersToProcess) {
            try {
                ...
                if (existing) {
                    ...
                    await updateEmployeeDb(user.id, updates);
                    updated++;
    ...
    const handleAdminUpdateAttendance = async (payload: { userId: string; date: string; entityId: string; status: "hadir" | "tidak-hadir" | null; reason: string | null; }) => {
        try {
            ...
            // Reload attendance data for this user from Database
            ...
            setAllUsersData((prev) => {
                ...
                if (newData[payload.userId]) {
                    // Update attendance field with fresh data from Database
    ...
    const handleUpdateProfile = async (userId: string, updates: Partial<Omit<Employee, 'id' | 'role' | 'password'>> | { functionalRoles: FunctionalRole[] }, silent: boolean = false) => {
        try {
            ...
            // Update in Database
            await updateEmployeeDb(userId, updates);
    ...
    const handleAddHospital = async (data: Omit<Hospital, 'id' | 'isActive'> & { logoFile?: File }) => {
        try {
            // Upload logo to Storage if file provided
            ...
            // Create in Database
            await createHospitalDb(hospitalData);
    ...
    const handleUpdateHospital = async (id: string, data: Partial<Omit<Hospital, 'id'>> & { logoFile?: File }) => {
        try {
            // Upload new logo to Storage if file provided
            let finalData = { ...data };
            if (data.logoFile) {
                const { uploadHospitalLogo } = await import('@/services/hospitalService');
            ...
            // Update in Database
            await updateHospitalDb(id, finalData);
    ...
    const handleToggleHospitalStatus = async (id: string) => {
        try {
            // Toggle in Database
            await toggleHospitalStatusDb(id);
