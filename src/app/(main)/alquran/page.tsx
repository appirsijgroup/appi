...
import { Alquran } from '@/components/features/quran/Alquran';
...
export default function AlquranPage() {
    ...
    const handleQuranReadingSubmission = async (details: {
        ...
        try {
            ...
            const selectedDateObj = new Date(details.date);
            if (today.getMonth() !== selectedDateObj.getMonth() || today.getFullYear() !== selectedDateObj.getFullYear()) {
                addToast(`⚠️ Tidak dapat melaporkan bacaan Al-Qur'an untuk bulan yang sudah terlewat. Periode pelaporan telah ditutup.`, 'error');
                return;
            }
            ...
            if (result) {
                ...
                if (updatedEmployee) {
                    ...
                    useAuthStore.setState({ loggedInEmployee: updatedEmployee });

                    useEmployeeStore.setState((state) => ({
                        allUsersData: {
                            ...state.allUsersData,
                            [updatedEmployee.id]: {
                                ...state.allUsersData[updatedEmployee.id],
