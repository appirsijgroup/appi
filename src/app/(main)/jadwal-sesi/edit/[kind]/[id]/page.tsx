...
export default function EditActivitySessionPage() {
    ...
    const allUsers = useMemo(() => Object.values(allUsersData || {}).map((d: any) => d.employee), [allUsersData]);
    ...
    useEffect(() => {
        const loadPageData = async () => {
            ...
            try {
                ...
                if (data) {
                    ...
                    if (data) {
                        // ⚡ ADD: Permission Check - Only creator or super-admin can edit
                        const isSuperAdmin = useAuthStore.getState().loggedInEmployee?.role === 'super-admin';
                        const currentEmpId = useAuthStore.getState().loggedInEmployee?.id;
                        const creatorId = (data as any).createdBy || (data as any).creatorId;
        ...
        loadPageData();
    }, [kind, id, activities, teamAttendanceSessions, router, loadAllEmployees, loadHospitals, addToast, loggedInEmployee]);
