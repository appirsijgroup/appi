...
import { PlusCircleIcon, CalendarIcon, UsersIcon, ClockIcon } from '@/components/ui/Icons';
...
export default function JadwalSesiPage() {
    ...
    const { activities, deleteActivity, teamAttendanceSessions, deleteTeamAttendanceSession, loadTeamAttendanceSessions, loadActivities } = useActivityStore();
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                await loadActivities(undefined, creatorId);
            } catch (error) {
        ...
        loadData();
    }, [loggedInEmployee, loadTeamAttendanceSessions, loadActivities]);
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            // ⚡ TAMBAH: Reload data dari Database setelah delete (tetap gunakan filter)
            const isSuperAdmin = loggedInEmployee?.role === 'super-admin';
            const creatorId = isSuperAdmin ? undefined : loggedInEmployee?.id;
            await loadTeamAttendanceSessions(creatorId);
            await loadActivities(undefined, creatorId);
