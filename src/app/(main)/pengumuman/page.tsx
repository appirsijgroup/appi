...
import { useAuthStore, useEmployeeStore, useHospitalStore, useAnnouncementStore, useUIStore } from '@/store/store';
import type { Announcement } from '@/types';
import MinimalistLoader from '@/components/ui/MinimalistLoader';
import { isAnyAdmin } from '@/lib/rolePermissions';

export default function PengumumanPage() {
    const { loggedInEmployee, markAnnouncementAsRead } = useAuthStore();
    const { allUsersData, loadAllEmployees } = useEmployeeStore();
    const { hospitalsData, loadHospitals } = useHospitalStore();
    ...
    const allUsersList = React.useMemo(() => {
        return Object.values(allUsersData || {}).map((data: { employee: any }) => data.employee);
    }, [allUsersData]);

    // Load announcements and hospitals from Database on mount
    useEffect(() => {
    ...
    // Handler to create announcement with proper structure and save to Database
