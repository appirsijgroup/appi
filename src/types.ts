...
export interface RawEmployee {
  ...
  employmentStatus?: 'Pegawai Tetap' | 'Kontrak' | 'Mitra' | 'Part Time';
  email?: string;
  role?: Role;
...
export type HistoryRecord = Record<string, DailyAttendance>;
...
export type AdminView = 'manajemen-pengguna' | 'manajemen-konten' | 'reports' | 'pengumuman' | 'manajemen-admin' | 'manajemen-rs' | 'reset-password';
...
export interface MyDashboardViewProps {
  ...
  onUpdateMenteeTargetStatus: (targetId: string, status: 'completed' | 'in-progress' | 'cancelled') => void;
  onDeleteMenteeTarget: (targetId: string) => void;
  confirmDeleteTarget?: MenteeTarget | null;
  ...
  onUpdateMonthlyActivities?: (userId: string, monthKey: string, monthProgress: MonthlyActivityProgress) => void;
