...
/**
 * Optimize signature image while preserving transparency
 * This ensures signatures look good on any background color
 * Converts to PNG with transparency support
 */
export const optimizeSignatureImage = async (imageSource: string): Promise<string> => {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => {
        ...
        img.onerror = (err) => {
            console.error('Signature optimization error:', err);
            reject(err);
        };
        img.src = imageSource;
    });
