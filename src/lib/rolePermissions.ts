...
export function isAdmin(user: Employee | null): boolean {
  ...
  return ['super-admin', 'admin'].includes(user.role) || !!user.canBeBPH || !!user.functionalRoles?.includes('BPH');
}
...
export function isSuperAdmin(user: Employee | null): boolean {
  if (!user) return false;
  return user.role === 'super-admin' || !!user.canBeBPH || !!user.functionalRoles?.includes('BPH');
}
